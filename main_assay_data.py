...
class DataObject(object):
    ...
    def __init__(self, file_name, batch_size, mini_batch_size):

        # Load in training data set

        data = h5py.File(file_name, 'r')

        self.data_x = data['x_activity'][...]
        self.data_y = data['y_activity'][...]
        ...
        self.data_x = numpy.concatenate((self.data_x, numpy.zeros((pad, self.dims[1]))), axis=0)
        self.data_y = numpy.concatenate((self.data_y, numpy.zeros((pad, self.data_y.shape[1]))), axis=0)
        self.dims = self.data_x.shape

        self.shared_x = theano.shared(numpy.asarray(self.data_x[:batch_size, :], dtype=theano.config.floatX), borrow=True)
    ...
    def new_shared(self, index):
        ...
        self.shared_x.set_value(numpy.asarray(self.data_x[pos1:pos2, :], dtype=theano.config.floatX), borrow=True)
...
class NeuralNetwork(object):
    ...
    def __init__(self, x, y, n_in, learning_rate=0.5):
        rng = numpy.random.RandomState(1234)

        # Construct network

        self.hiddenLayer1 = HiddenLayer(rng=rng, input=x, n_in=n_in, n_out=50)
        self.classifier = CrossEntropyLoss(input=self.hiddenLayer1.output, n_in=50, n_out=7)

        # combine network paramters and momentum terms
...
def run_network(n_epochs=35, mini_batch_size=200):
    ...
    train_data = DataObject('activities_train.hdf5', 110600, mini_batch_size)
    test_data = DataObject('activities_test.hdf5', 115000, mini_batch_size)
